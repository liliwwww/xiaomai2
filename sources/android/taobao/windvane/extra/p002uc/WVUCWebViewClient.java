...
public class WVUCWebViewClient extends WebViewClient {
    ...
    private Runnable mCrashCountReseter = new Runnable() { // from class: android.taobao.windvane.extra.uc.WVUCWebViewClient.5
        ...
        public void run() {
            TaoLog.m21e(WVUCWebViewClient.SANDBOX_TAG, "crash count reset - " + WVUCWebViewClient.this.crashCount);
                ...
                try {
                    ...
                    if (metaObject != null && wVUCWebView != null) {
                        if (!metaObject.has("WV.Meta.Performance.JSFSP")) {
                            wVUCWebView.setTag(ViewToken.APM_VIEW_TOKEN, "valid_view");
                            TaoLog.m18d(WVUCWebViewClient.TAG, "no JSFSP setTag " + SystemClock.uptimeMillis());
                        } else if (TextUtils.isEmpty(metaObject.optString("WV.Meta.Performance.JSFSP"))) {
                            wVUCWebView.setTag(ViewToken.APM_VIEW_TOKEN, "valid_view");
                            TaoLog.m18d(WVUCWebViewClient.TAG, "no version setTag " + SystemClock.uptimeMillis());
                        } else if (!wVUCWebView.isReportedFSP()) {
                            wVUCWebView.setTag(ViewToken.APM_VIEW_TOKEN, "invalid_view");
                        }
                    }
                } catch (Throwable th) {
                    th.printStackTrace();
    ...
    */
    private com.uc.webview.export.WebResourceResponse shouldInterceptRequestInternal(com.uc.webview.export.WebView r11, java.lang.String r12, android.taobao.windvane.service.WVEventResult r13) {
        ...
        */
        throw new UnsupportedOperationException("Method not decompiled: android.taobao.windvane.extra.p002uc.WVUCWebViewClient.shouldInterceptRequestInternal(com.uc.webview.export.WebView, java.lang.String, android.taobao.windvane.service.WVEventResult):com.uc.webview.export.WebResourceResponse");
    }
    ...
    public void onPageFinished(final WebView webView, final String str) {
        String str2;
        TaoLog.m21e(TAG, "onPageFinished : " + str);
        ...
        try {
            if (webView instanceof WVUCWebView) {
                ...
                TaoLog.m21e(TAG, "view is " + webView);
            }
        } catch (Throwable th) {
            th.printStackTrace();
        }
        ...
        TaoLog.m24i(TAG, str + " LayerType : " + webView.getLayerType());
        if (webView.getCurrentViewCoreType() == 2) {
            webView.evaluateJavascript("javascript:(function(f){if(f.__windvane__.call) return true; else return false})(window)", new ValueCallback<String>() { // from class: android.taobao.windvane.extra.uc.WVUCWebViewClient.3
                @Override // android.webkit.ValueCallback
                public void onReceiveValue(String str3) {
                    TaoLog.m24i("WVJsBridge", "has windvane :" + str3);
        ...
        if (WebView.getCoreType() == 1 || WebView.getCoreType() == 3) {
            TaoLog.m18d(TAG, "onPageFinished.  core type = " + WebView.getCoreType());
    ...
    public void onPageStarted(WebView webView, String str, Bitmap bitmap) {
        ...
        TaoLog.m21e(TAG, "onPageStarted : " + str);
    }

    public void onReceivedError(WebView webView, int i, String str, String str2) {
        FalcoSpan falcoSpan;
        if (TaoLog.getLogStatus()) {
            TaoLog.m21e(TAG, "Receive error, code: " + i + "; desc: " + str + "; url: " + str2);
        }
    ...
    public void onReceivedSslError(WebView webView, SslErrorHandler sslErrorHandler, SslError sslError) {
        ...
        if (TaoLog.getLogStatus()) {
            TaoLog.m21e(TAG, "onReceivedSslError  url: " + sslError.getUrl() + "errorMsg:" + sslError2);
        }
        String url = webView.getUrl();
        if (webView instanceof WVUCWebView) {
            HashMap hashMap = new HashMap(2);
    ...
    public boolean onRenderProcessGone(final WebView webView, RenderProcessGoneDetail renderProcessGoneDetail) {
        TaoLog.m21e(SANDBOX_TAG, "onRenderProcessGone webview:" + webView + ", crash:" + renderProcessGoneDetail.didCrash() + ", priority:" + renderProcessGoneDetail.rendererPriorityAtExit());
        ...
        if (webView == null) {
            TaoLog.m21e(SANDBOX_TAG, "onRenderProcessGone - WebView is null");
            return false;
        }
        this.crashCount = i + 1;
    ...
    public WebResourceResponse shouldInterceptRequest(WebView webView, WebResourceRequest webResourceRequest) {
        if (WebView.getCoreType() != 3) {
            TaoLog.m21e(TAG, "Only U4 WebView will use shouldInterceptRequest(API Level 21), return null.");
            return null;
        }
        if (webResourceRequest == null || webResourceRequest.getUrl() == null) {
            TaoLog.m21e(TAG, "shouldInterceptRequest, invalid request.");
            return null;
        }
        ...
        if (webView instanceof IWVWebView) {
            return shouldInterceptRequestInternal(webView, uri, WVEventService.getInstance().onEvent(1008, (IWVWebView) webView, uri, webResourceRequest.getRequestHeaders(), Integer.valueOf((!(webView instanceof WVUCWebView) || ((WVUCWebView) webView).getUCExtension() == null) ? 2 : 3)));
        }
        return null;
    }
    ...
    public boolean shouldOverrideUrlLoading(WebView webView, String str) {
        try {
            ...
            TaoLog.m21e(TAG, "getSpan failed " + CommonUtils.getStackTrace(th));
        }
        ...
        if (WVURLFilter.doFilter(str, this.mContext.get(), iWVWebView)) {
            TaoLog.m21e(TAG, "shouldOverrideUrlLoading filter url=" + str);
        ...
        if (str.startsWith("mailto:") || str.startsWith(SCHEME_TEL) || str.startsWith(SCHEME_SMS)) {
            try {
                ...
                TaoLog.m21e(TAG, "shouldOverrideUrlLoading: ActivityNotFoundException, url=" + str);
            }
            return true;
        }
        WVUCWebView wVUCWebView = (WVUCWebView) webView;
        ...
        try {
            if ((webView instanceof IWVWebView) && WVURLInterceptService.getWVURLIntercepter() != null && WVURLInterceptService.getWVURLIntercepter().isOpenURLIntercept()) {
                ...
                if (WVURLInterceptService.getWVURLIntercepter().shouldOverrideUrlLoading(context, (IWVWebView) webView, str)) {
                    TaoLog.m24i(TAG, "intercept url : " + str);
                    return true;
                }
            }
        } catch (Exception e) {
            TaoLog.m21e(TAG, "shouldOverrideUrlLoading: doFilter error, " + e.getMessage());
        }
        if (webView instanceof WVUCWebView) {
            UCNetworkDelegate.getInstance().onUrlChange(wVUCWebView, str);
            wVUCWebView.setCachedUrl(str);
        }
        TaoLog.m24i(TAG, "shouldOverrideUrlLoading : " + str);
        try {
            HashMap hashMap2 = new HashMap();
            hashMap2.put("userAgent", webView.getSettings().getUserAgentString());
            WMLPrefetch.getInstance().prefetchData(str, hashMap2);
        } catch (Throwable th2) {
            TaoLog.m21e(TAG, "failed to call prefetch: " + th2.getMessage());
            th2.printStackTrace();
        }
    ...
    public WebResourceResponse shouldInterceptRequest(WebView webView, String str) {
        if (WebView.getCoreType() == 3) {
            TaoLog.m21e(TAG, "U4 WebView will not use shouldInterceptRequest(API Level 11), return null.");
            return null;
        }
        if (!(webView instanceof IWVWebView)) {
            return null;
