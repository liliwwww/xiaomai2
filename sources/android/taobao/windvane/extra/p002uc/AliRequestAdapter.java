...
public class AliRequestAdapter implements IRequest {
    ...
    public void cancel() {
        ...
        TaoLog.m21e(this.TAG, "cancel id= " + this.mEventHandler.hashCode() + ", phase:[" + this.cancelPhase + "]");
        try {
            if (TaoLog.getLogStatus() && (future = this.mFutureResponse) != null && future.get() != null) {
                TaoLog.m18d(this.TAG, "AliRequestAdapter cancel desc url=" + this.mFutureResponse.get().getDesc());
            }
            complete();
        } catch (InterruptedException e) {
            e.printStackTrace();
            TaoLog.m18d(this.TAG, "AliRequestAdapter cancel =" + e.getMessage());
        } catch (ExecutionException e2) {
            e2.printStackTrace();
            TaoLog.m18d(this.TAG, "AliRequestAdapter cancel =" + e2.getMessage());
        }
        ...
        if (this.mEventHandler.isSynchronous()) {
            synchronized (this.mClientResource) {
                if (TaoLog.getLogStatus()) {
                    TaoLog.m18d(this.TAG, "AliRequestAdapter complete will notify");
                }
    ...
    public void waitUntilComplete(int i) {
        if (this.mEventHandler.isSynchronous()) {
            synchronized (this.mClientResource) {
                try {
                    if (TaoLog.getLogStatus()) {
                        TaoLog.m18d(this.TAG, "AliRequestAdapter waitUntilComplete timeout=" + i + ",url=" + this.mUrl);
                    }
            ...
            try {
                if (CommonUtils.isPicture(str)) {
                    ...
                    if (!TextUtils.isEmpty(justConvergeAndWebP) && !justConvergeAndWebP.equals(str)) {
                        TaoLog.m24i(this.TAG, str + " decideUrl to : " + justConvergeAndWebP);
        ...
        try {
            ...
            if (map != null) {
                ...
                for (Map.Entry<String, String> entry : map.entrySet()) {
                    ...
                    if (TaoLog.getLogStatus()) {
                        TaoLog.m18d(this.TAG, "AliRequestAdapter from uc header key=" + key + ",value=" + value);
                    }
            ...
            TaoLog.m21e(this.TAG, " AliRequestAdapter formatAliRequest Exception" + e.getMessage());
            return null;
        }
    }
