...
public class ProcessLockUtil {
    ...
    private void closeQuietly(Closeable closeable) {
        if (closeable != null) {
            try {
                ...
                TaoLog.m22e(TAG, "Failed to close resource", e, new Object[0]);
            }
        }
    }
    ...
    public void lock() {
        try {
            ...
            if (this.lockFile == null) {
                TaoLog.m21e(TAG, "lock error lockRaf = " + this.lockRaf + " lockFile = " + this.lockFile);
                return;
            }
            this.lockChannel = randomAccessFile.getChannel();
            TaoLog.m18d(TAG, "Blocking on lock " + this.lockFile.getPath());
            try {
                this.cacheLock = this.lockChannel.lock();
                TaoLog.m18d(TAG, this.lockFile.getPath() + " locked");
            } catch (IOException e) {
                TaoLog.m22e(TAG, "lock error ", e, new Object[0]);
            }
        } catch (FileNotFoundException e2) {
            TaoLog.m22e(TAG, "ProcessLock error", e2, new Object[0]);
        }
    }
    ...
    public void unlock() {
        ...
        if (this.lockFile != null) {
            TaoLog.m18d(TAG, this.lockFile.getPath() + " unlocked");
        }
    }
