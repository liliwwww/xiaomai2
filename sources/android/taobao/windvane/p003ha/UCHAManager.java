...
public class UCHAManager implements ValueCallback<Pair<Message, Message>> {
    ...
    public void onReceiveValue(Pair<Message, Message> pair) {
        ...
        if (z) {
            TaoLog.m21e(TAG, "collect log, type:[" + str + "]; linkID:[" + str4 + "]; logData:[" + str2 + "]");
        } else {
            TaoLog.m24i(TAG, "need not collect log, type:[" + str + "]; linkID:[" + str4 + "];logData:[" + str2 + "]");
        }
        ...
        if (i == 2) {
            ...
            TaoLog.m24i(TAG, "webgl error report with " + wVCommonConfigData.ucParam.webglErrorRate + "% sample rate");
            z = z2;
        }
        if (z) {
