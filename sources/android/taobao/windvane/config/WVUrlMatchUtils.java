...
public class WVUrlMatchUtils {
    ...
    private int computeGravitySum(ArrayList<String> arrayList) {
        ...
        for (int i2 = 0; i2 < arrayList.size(); i2++) {
            if (arrayList.get(i2).equals("*") || arrayList.get(i2).equals("+")) {
                ...
                if (arrayList.get(i2).equals(WVNativeCallbackUtil.SEPERATER)) {
                    z = true;
                }
                i = z ? i + 10 : i + 1000;
            }
    ...
    private UrlParseRes parseUrl(String str) {
        ...
        if (split.length > 0 && TextUtils.equals("", split[0])) {
            urlParseRes.subPaths[0] = WVNativeCallbackUtil.SEPERATER;
        }
        if (urlParseRes.subPaths.length == 0) {
            urlParseRes.subPaths = new String[]{WVNativeCallbackUtil.SEPERATER};
        }
    ...
    private void travelTree(UrlParseRes urlParseRes, boolean z, TireNode tireNode, ArrayList<String> arrayList, int i, Map<ArrayList<String>, String> map) {
        ...
        if (tireNode.subField != null && z) {
            if (i3 == urlParseRes.subPaths.length) {
                ...
                if (map2 == null || !map2.containsKey("*") || tireNode.nodeMap.get("*").group == null || !checkSchemeAndPort(urlParseRes, tireNode.nodeMap.get("*"))) {
                    return;
                }
                arrayList.add("*");
                map.put(new ArrayList<>(arrayList), tireNode.nodeMap.get("*").group);
                arrayList.remove("*");
                return;
            }
            if (TextUtils.equals(tireNode.subField, WVNativeCallbackUtil.SEPERATER) && tireNode.group != null && checkSchemeAndPort(urlParseRes, tireNode)) {
                map.put(new ArrayList<>(arrayList), tireNode.group);
            }
        }
        ...
        int i6 = i3;
        if (tireNode.nodeMap.containsKey("+")) {
            if (z2) {
                z4 = z2;
            ...
            arrayList.add("+");
            travelTree(urlParseRes, z4, tireNode.nodeMap.get("+"), arrayList, length2, map);
            arrayList.remove("+");
            if (length2 == 0) {
                z4 = false;
            }
        ...
        if (tireNode.nodeMap.containsKey("*")) {
            if (z3 && i6 == 0 && TextUtils.equals(lowerCase, WVNativeCallbackUtil.SEPERATER)) {
                arrayList.add("*");
                travelTree(urlParseRes, z3, tireNode.nodeMap.get("*"), arrayList, i6, map);
                arrayList.remove("*");
                return;
            }
            if (z3) {
            ...
            arrayList.add("*");
            travelTree(urlParseRes, z3, tireNode.nodeMap.get("*"), arrayList, length, map);
            arrayList.remove("*");
        }
    }
    ...
    boolean checkSchemeAndPort(UrlParseRes urlParseRes, TireNode tireNode) {
        ...
        if (TextUtils.equals(scheme, strArr[0])) {
            ...
            while (true) {
                ...
                if (TextUtils.equals(strArr[i], urlParseRes.scheme)) {
                    z2 = true;
                    break;
                }
                i++;
    ...
    public String searchUrl(String str) {
        ...
        while (true) {
            ...
            if (i2 < strArr.length) {
                ...
                if (tireNode3 == null) {
                    if (!tireNode2.nodeMap.containsKey("*")) {
                        if (!tireNode2.nodeMap.containsKey("+")) {
                            break;
                        }
                        tireNode2 = tireNode2.nodeMap.get("+");
        ...
        if (!tireNode2.nodeMap.containsKey(WVNativeCallbackUtil.SEPERATER) && tireNode2.nodeMap.containsKey("*")) {
            tireNode2 = tireNode2.nodeMap.get("*");
        }
        int i3 = 0;
        while (true) {
            ...
            if (i3 < strArr2.length) {
                ...
                if (tireNode4 == null) {
                    if (!tireNode2.nodeMap.containsKey("*")) {
                        if (!tireNode2.nodeMap.containsKey("+")) {
                            if (!tireNode2.subField.equals(WVNativeCallbackUtil.SEPERATER) || tireNode2.group == null) {
                                break;
                            }
        ...
        if (tireNode2.group == null && tireNode2.nodeMap.containsKey("*")) {
            tireNode2 = tireNode2.nodeMap.get("*");
        }
