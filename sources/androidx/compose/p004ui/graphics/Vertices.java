...
public final class Vertices {
    ...
    private final int[] encodeColorList(List<Color> list) {
        ...
        for (int i = 0; i < size; i++) {
            iArr[i] = ColorKt.m2841toArgb8_81llA(list.get(i).m2797unboximpl());
        }
        return iArr;
    }
    ...
    private final float[] encodePointList(List<Offset> list) {
        ...
        for (int i = 0; i < size; i++) {
            long m2566unboximpl = list.get(i / 2).m2566unboximpl();
            fArr[i] = i % 2 == 0 ? Offset.m2556getXimpl(m2566unboximpl) : Offset.m2557getYimpl(m2566unboximpl);
        }
        return fArr;
    }
    ...
    public final int m3141getVertexModec2xauaI() {
